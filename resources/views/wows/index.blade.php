        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('wows.store') }}">
                        @csrf 
                        ...
                        <x-primary-button>WoW</x-primary-button>
                    </form>
                </div>
            </div>
                    ...
                    <div class="p-6 flex space-x-2">
                        ...
                        <div class="flex-1">
                            <div class="flex justify-between items-center">
                                <div>
                                    <span class="text-gray-800 dark:text-gray-200">{{ $wow->user->name }}</span>
                                    <small class="ml-2 text-sm text-gray-600 dark:text-gray-400">{{ $wow->created_at->format('j M Y, g:i a') }}</small>

                                    @unless($wow->created_at->eq($wow->updated_at))
                                        <small class="text-sm text-gray-600 dark:text-gray-400"> &middot; {{ __('edited') }}</small>
                                    @endunless
                                    
                                </div>
                            </div>
                                <p class="mt-4 text-lg text-gray-900 dark:text-gray-100">{{ $wow->message }}</p>
                            </div>
                            ...
                            @can('update', $wow)
                                <x-dropdown>
                                    ...
                                    <x-slot name="content">
                                    <x-dropdown-link :href="route('wows.edit', $wow)">
                                    {{ __('Edit Wow') }}
                                    </x-dropdown-2link>
                                    <form method="POST" action="{{ route('wows.destroy', $wow) }}">
                                        @csrf @method('DELETE')
                                        <x-dropdown-link :href="route('wows.destroy', $wow)" onclick="event.preventDefault(); this.closest('form').submit();">
                                            {{ __('Delete Wow') }}
                                        </x-dropdown-link>
                                    </form>
